...
# =============================
# TWITTER API V2 ONLY (Supports media upload)
# =============================

def upload_media_to_twitter(image_bytes, client, auth=None):
    ...
    try:
        ...
        if hasattr(media_response, 'media_id_string'):
            return media_response.media_id_string
        elif hasattr(media_response, 'media_id'):
            return str(media_response.media_id)
        elif hasattr(media_response, 'data'):
            return media_response.data['media_id_string']
        else:
            print(f"Unexpected media response format: {media_response}")
            return None
...
def post_to_twitter_with_image(content, image_bytes, api_key, api_secret, access_token, access_token_secret):
    ...
    try:
        ...
        # Upload image
        media_id = upload_media_to_twitter(image_bytes, client)
...
def generate_engaging_post():
    ...
    try:
        ...
        if len(hashtags) > 3:
            ...
            tweet_text = ' '.join(tweet_lines)
            additional_tags = extract_hashtags_from_text(tweet_text, max_tags=3-len(hashtags))
            hashtag_line = f"{hashtag_line} {additional_tags}".strip()
